...
type operation struct {
	...
	BodyFile    string // `< file` reference used instead of Body when the example contains {{
...
// reGenerated matches the placeholders the importer itself emits for
// uuid and date-time formats; those are meant to be rendered.
var reGenerated = regexp.MustCompile(`\{\{\$(uuid|isoTimestamp)\}\}`)

// hasLiteralBraces reports whether a body carries {{ from the spec's own
// examples, which the runner would otherwise treat as a template.
func hasLiteralBraces(body string) bool {
	return strings.Contains(reGenerated.ReplaceAllString(body, ""), "{{")
}
